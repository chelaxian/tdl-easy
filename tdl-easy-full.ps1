...
if (-not $useSaved) {
    ...
    try {
        ...
        do {
            ...
            Write-Host "║ Example: https://t.me/c/12345678/123 (any message from the chat)" -ForegroundColor Gray
            Write-Host "╠────────────────────────────────────────────────────────────────────────────╣" -ForegroundColor DarkGray
...
# Extract chat ID from message URL
$channelId = $null
if ($telegramMessageUrl -match '^https?://t\.me/c/(\d+)/\d+$') {
    $channelId = $Matches[1]
}
if ([string]::IsNullOrWhiteSpace($channelId)) {
    Write-Host "🔴 Error: Failed to extract chat ID from URL: $telegramMessageUrl" -ForegroundColor Red
    while ($true) {
        $resp = Read-Host "Enter Telegram message URL again in form https://t.me/c/12345678/123 or type 'quit' to exit"
        ...
        if ($resp -match '^https?://t\.me/c/(\d+)/\d+$') {
            ...
            Write-Host "🔴 Invalid format, must be https://t.me/c/12345678/123 exactly." -ForegroundColor Red
        }
    }
}
...
# Export all messages to tdl-export.json
