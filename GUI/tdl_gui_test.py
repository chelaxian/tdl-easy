...
MAIN_ROOT = None
LOGIN_CACHE_FILE = "tdl_easy_login.json"
...
def find_shell():
    """
    Return full path to pwsh.exe if available, otherwise 'powershell.exe'.
    """
    pwsh = shutil.which("pwsh")
    return pwsh if pwsh else "powershell.exe"

def clean_all_scripts():
    """
    Remove non-ASCII characters (e.g. ✓) from all .ps1 scripts in launcher dir.
    """
    d = get_launcher_dir()
...
def run_powershell_script(script_path, extra_command=None):
    ...
    shell = find_shell()
    # if falling back to Windows PowerShell, clean scripts first
...
def login_telegram():
    ...
    messagebox.showinfo(
        "Логин в Telegram",
        "Откроется консоль. Выберите свой user id, затем на вопрос\n"
        "'Do you want to logout existing desktop session?' ответьте N."
    )
    shell = find_shell()
    if os.path.basename(shell).lower() == "powershell.exe":
        clean_all_scripts()
...
def make_autoyes_wrapper(original_name, wrapper_name):
    ...
    wrapper_path = os.path.join(launcher_dir, wrapper_name)
    content = (
        "# Auto wrapper: answer Yes to saved parameters prompt and invoke original\n"
...
def download_range():
    ...
    tdl_path = dlg.result.strip() if dlg.result else default
    ...
    media_dir = dlg2.result.strip() if dlg2.result else default
    if not os.path.exists(media_dir):
        messagebox.showerror("Ошибка", f"Media directory не найден: {media_dir}")
        return

    while True:
        base_link = simpledialog.askstring("Базовая ссылка Telegram", "https://t.me/c/12345678/ или https://t.me/username/:", parent=MAIN_ROOT)
        if not base_link:
            return
        base_link = base_link.strip()
        if not base_link.endswith("/"):
            base_link += "/"
        if re.match(r"^https?://t\.me/c/\d+/$", base_link) or re.match(r"^https?://t\.me/[A-Za-z0-9_]+/$", base_link):
            break
        messagebox.showwarning("Неправильный формат", "Ожидается корректная Telegram ссылка с '/' в конце.")

    # start/end IDs
    start_id = IntegerInputDialog(MAIN_ROOT, "Начальный индекс", "Введите startId:", initialvalue=1, minvalue=1).result
    if start_id is None: return
    end_id = IntegerInputDialog(MAIN_ROOT, "Конечный индекс", f"Введите endId (>= {start_id}):", initialvalue=start_id+99, minvalue=start_id).result
    if end_id is None: return

    dl_limit = IntegerInputDialog(MAIN_ROOT, "Лимит задач", "Concurrent tasks (1–10):", initialvalue=2, minvalue=1, maxvalue=10).result
    if dl_limit is None: return
    threads = IntegerInputDialog(MAIN_ROOT, "Потоки", "Threads per task (1–8):", initialvalue=4, minvalue=1, maxvalue=8).result
    if threads is None: return

    state = {
        "tdl_path": tdl_path, "telegramUrl": base_link, "mediaDir": media_dir,
        "startId": start_id, "endId": end_id,
        "downloadLimit": dl_limit, "threads": threads, "maxRetries": 1
    }
    state_file = os.path.join(launcher_dir, "tdl_easy_runner.json")
    if not write_state_json(state_file, state): return
...
def download_full_chat():
    ...
    msg_url = simpledialog.askstring("Ссылка на сообщение", "Введите URL (https://t.me/.../123):", parent=MAIN_ROOT)
    if not msg_url:
        return
    ...
    state = {
        "tdl_path": tdl_path, "telegramMessageUrl": msg_url, "mediaDir": media_dir,
        "downloadLimit": dl_limit, "threads": threads, "maxRetries": 1
    }
    state_file = os.path.join(launcher_dir, "tdl_easy_runner.json")
    if not write_state_json(state_file, state): return
...
def build_ui():
    ...
    tk.Label(frm, text="Меню:", font=("Segoe UI", 14, "bold")).grid(row=0, column=0, columnspan=2, pady=(0,8), sticky="w")
    tk.Button(frm, text="УСТАНОВИТЬ/ОБНОВИТЬ TDL", width=35, command=install_update_tdl).grid(row=1, column=0, pady=4, padx=4)
    tk.Button(frm, text="ЛОГИН В TELEGRAM", width=35, command=login_telegram).grid(row=2, column=0, pady=4, padx=4)
    tk.Button(frm, text="СКАЧАТЬ ОДИНОЧНЫЙ ФАЙЛ", width=35, command=download_single_file).grid(row=3, column=0, pady=4, padx=4)
    tk.Button(frm, text="СКАЧАТЬ ДИАПАЗОН ПОСТОВ", width=35, command=download_range).grid(row=4, column=0, pady=4, padx=4)
    tk.Button(frm, text="СКАЧАТЬ ВСЁ ИЗ ЧАТА", width=35, command=download_full_chat).grid(row=5, column=0, pady=4, padx=4)
    tk.Button(frm, text="ВЫХОД", width=35, command=root.destroy).grid(row=6, column=0, pady=(12,4), padx=4)

    tk.Label(frm, text="PowerShell-окна остаются открытыми для просмотра/ввода параметров.", font=("Segoe UI", 8), fg="gray").grid(row=7, column=0, columnspan=2, pady=(8,0))
    root.mainloop()
